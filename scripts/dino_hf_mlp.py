...
def get_image_embeddings(model, processor, batch_size, df, device: str = "cuda:0"):
    ...
    for i in tqdm(range(0, len(df), batch_size)):
        ...
        with torch.no_grad():
            ...
            image_emb = outputs["pooler_output"]  # (batch_size, emb_size)
            hidden_states = outputs[
                "hidden_states"
            ]  # List of (batch_size, timestamps, emb_size)
            pooled_hs = [
                torch.cat((h.mean(dim=1), h.std(dim=1)), dim=-1) for h in hidden_states
            ]
            pooled_hs = torch.stack(
                pooled_hs, dim=1
            )  # (batch_size, num_layer, emb_size)
        image_embeddings.append(image_emb.detach().cpu())
        all_pooled_hs.append(pooled_hs.detach().cpu())
    image_embeddings = torch.cat(image_embeddings, dim=0)
    all_pooled_hs = torch.cat(all_pooled_hs, dim=0)
    return image_embeddings, all_pooled_hs
